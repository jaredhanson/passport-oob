  ...
  if (code) {
    ...
    try {
      if (self._passReqToCallback) {
        ...
        if (arity == 6) {
          this._verify(req, address, transport, code, req.state, verified);
        } else if (arity == 5) {
        ...
        var arity = self._verify.length;
        if (arity == 5) {
          this._verify(address, transport, code, req.state, verified);
        } else if (arity == 4) {
    ...
    function transmitted(err, ctx) {
      ...
      var state = ctx || {};
      state.address = ctx.address || address;
      ...
      req.pushState(state, self._verifyURL, function(err, h) {
        ...
        console.log('REDIRECTING...: ' + self._verifyURL);
        console.log('h: ' + h);
