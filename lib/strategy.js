...
Strategy.prototype.authenticate = function(req, options) {
  ...
  if (!req.state) { return this.error(new Error('Out-of-band authentication requires state support. Did you forget to use `flowstate` middleware?')); }
  ...
  if (!address || !secret) {
    return this.error(new Error('Invalid authentication request state.'));
  }
